...
def templates_list(request):
    """List available email templates found in the repository 'email-templates-master' folder."""
    templates_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '../../../../email-templates-master'))
    templates = []
    if os.path.isdir(templates_dir):
        for name in sorted(os.listdir(templates_dir)):
            path = os.path.join(templates_dir, name)
            index = os.path.join(path, 'index.html')
            if os.path.isdir(path) and os.path.isfile(index):
                templates.append({'name': name})

    return render(request, 'campaigns/templates_list.html', {'templates': templates})


@login_required
def template_preview(request, template_name):
    """Return raw HTML for the requested template (index.html) so it can be previewed."""
    templates_dir = os.path.normpath(os.path.join(os.path.dirname(__file__), '../../../../email-templates-master'))
    index = os.path.join(templates_dir, template_name, 'index.html')
    ...
    try:
        ...
        return HttpResponse('Could not read template', status=500)
